...
按 [四卡消融说明](ABLATION_4GPU.md) 设置 `DATA_DIR`、`ARTIFACTS_DIR`、`AUTOENCODER`、`COHORT` 和四张获配 GPU，目录均使用 NAS 上的绝对路径。

```bash
bash scripts/nas.sh bash scripts/ablation_4gpu.sh prepare
bash scripts/nas.sh bash scripts/ablation_4gpu.sh preflight all
bash scripts/nas.sh bash scripts/ablation_4gpu.sh train all
```
