...
export RESULT_ROOT="$NAS_ROOT/runs/airfoil_dit_seed0"
export OUTPUT_DIR="$NAS_ROOT/results/airfoil_dit_seed0_sampling"
export ARTIFACTS_DIR=/path/to/prepared/airfoil_vgae
...
入口读取训练运行的已完成阶段选优记录，使用其中的 checkpoint 与 raw/EMA 权重，并核验配套 VGAE 表示和归一化。默认每次进行六步 DDIM。`--sampling-steps 20` 可在新的输出目录比较二十步采样。该参数与独立生成次数 K 分别记录。
