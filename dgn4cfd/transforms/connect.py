...
def connect_knn(
    pos: torch.Tensor, k: int, period: Tuple[float] = None
    ...
    if dx is not None:  # If periodicity is stablished along the x-axis
        ...
        x = torch.stack(
            (
                torch.cos(2 * np.pi / dx * pos[:, 0]),
                torch.sin(2 * np.pi / dx * pos[:, 0]),
            ),
            dim=1,
        )
    ...
    if dy is not None:  # If periodicity is stablished along the y-axis
        ...
        y = torch.stack(
            (
                torch.cos(2 * np.pi / dy * pos[:, 1]),
                torch.sin(2 * np.pi / dy * pos[:, 1]),
            ),
            dim=1,
        )
    ...
    if dim == 3:
        if dz is not None:  # If periodicity is stablished along the z-axis
            ...
            z = torch.stack(
                (
                    torch.cos(2 * np.pi / dz * pos[:, 2]),
                    torch.sin(2 * np.pi / dz * pos[:, 2]),
                ),
                dim=1,
            )
    ...
    if dx is not None:
        edge_attr[edge_attr[:, 0] < -dx / 2.0, 0] += dx
        edge_attr[edge_attr[:, 0] > dx / 2.0, 0] -= dx
    if dy is not None:
        edge_attr[edge_attr[:, 1] < -dy / 2.0, 1] += dy
        edge_attr[edge_attr[:, 1] > dy / 2.0, 1] -= dy
    if dim == 3 and dz is not None:
        edge_attr[edge_attr[:, 2] < -dz / 2.0, 2] += dz
        edge_attr[edge_attr[:, 2] > dz / 2.0, 2] -= dz
...
class ConnectKNN:
    ...
    def __call__(
        ...
        graph.edge_index, graph.edge_attr = connect_knn(
            graph.pos, self.k, period=self.period
        )
        return graph
