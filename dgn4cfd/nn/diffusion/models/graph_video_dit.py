...
if TYPE_CHECKING:
    from .h1_flex_attention import FlexLayout
...
class JointDiTBlock(nn.Module):
    ...
    def forward(
        ...
        flex_layout: FlexLayout | None = None,
    ) -> torch.Tensor:
        ...
        if flex_layout is not None:
            from .h1_flex_attention import attend

            batch, tokens_count, width = attention_input.shape
            query, key, value = F.linear(
                attention_input,
            ...
            def permute(tensor: torch.Tensor) -> torch.Tensor:
                return (
                    tensor.index_select(1, flex_layout.order)
                    .reshape(
                        batch,
                        tokens_count,
                        self.attention.num_heads,
                        width // self.attention.num_heads,
                    )
                    .transpose(1, 2)
                    .contiguous()
                )

            output = attend(permute(query), permute(key), permute(value), flex_layout)
            attention_output = self.attention.out_proj(
                output.transpose(1, 2).reshape(batch, tokens_count, width)
            )
        elif neighbor_layout is not None:
...
class GraphVideoDiT(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        attention_backend: Literal["neighbor", "flex"] = "neighbor",
    ) -> None:
        super().__init__()
        ...
        if attention_backend not in ("neighbor", "flex"):
            raise ValueError("attention_backend must be neighbor or flex")
        if attention_backend == "flex" and (
            attention_mode != "graph_hop_mask"
            or graph_hop_limit != 1
            or future_frames != 64
        ):
            raise ValueError("Flex is supported only for H1 with 64 future frames")
        self.attention_backend = attention_backend
        self.flex_layout_cache = None
        if attention_backend == "flex":
            from .h1_flex_attention import FlexLayoutCache

            self.flex_layout_cache = FlexLayoutCache()
    ...
    def architecture(self) -> dict[str, int | float | str]:
        ...
        if self.attention_backend != "neighbor":
    ...
    def forward(
        ...
        flex_layout = None
        attention_bias = None
        if self.flex_layout_cache is not None:
            flex_layout = self.flex_layout_cache.get(graph_hops, self.total_slots)
        elif self.attention_mode == "graph_hop_mask" and self.graph_hop_limit == 1:
