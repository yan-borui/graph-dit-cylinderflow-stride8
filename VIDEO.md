...
画面按方法分列，六行依次为速度模长、UV 向量误差、去均值压力、压力误差、涡量和涡量误差。压力逐帧去除全域面积加权均值，涡量使用三角形线性场的空间导数。每一物理量在全部方法与全部 65 帧之间共享固定色标，误差共享独立线性色标。
...
Airfoil 配置启用 `"vorticity_clip": {"quantile": 0.995}`。从当前轨迹真实场的全部 65 帧计算涡量绝对值的面积加权 99.5% 分位数 $L$，以 $[-L,L]$ 映射涡量颜色。各帧等权，与视窗相交的三角形按完整面积加权。涡量误差取每个方法的同一分位数，再以其中最大值设置所有方法共用的误差上限。分位数计算始终使用全部帧，包括导出三帧预览时。

超限值使用端点颜色，色条以箭头和 `clipped` 标注截断。原始数组、涡量计算和评价指标保持原样。色标记录保存每个面板的原始极值、逐帧上下超限面积比例、平均与最大超限面积比例；审核时结合这些比例判断是否遮住了大范围误差。99.5% 是预览起点，可调整 `quantile`，或以物理单位 $\mathrm{s}^{-1}$ 指定 `limit` 和 `error_limit`：

```json
"vorticity_clip": {"quantile": 0.995, "limit": 1000.0, "error_limit": 100.0}
```

上面的手动阈值仅展示参数用法，正式阈值需结合原始场确定。删除 `vorticity_clip` 或设为 `null` 时使用完整极值范围。CylinderFlow 和注意力配置沿用完整范围，也支持相同选项。

保持当前 Airfoil 轨迹的预览命令如下；沿用实验端配置中的现有预测路径，只加入裁剪选项，并选择新的输出目录：

```bash
python export_video.py --config configs/video/airfoil.json --trajectory 1018 --frames 0 32 64 --output-dir reports/airfoil_clip_preview
```
